...
#ifdef HAVE_OPENSSL
/* This mimics the server-side PASSWORD() SQL, which is:
 * "*" + "hex(sha1(sha1(password)))" + "\0"
 * length is 2*SHA_DIGEST_LENGTH + 2
 */
#define PAM_MYSQL_PASSWORD
static void pam_mysql_password(const unsigned char *d, unsigned int sz, char *md)
{
  int i;
  unsigned char hash1[SHA_DIGEST_LENGTH], hash2[SHA_DIGEST_LENGTH];

  SHA1(d, sz, hash1);
  SHA1(hash1, sizeof(hash1), hash2);
  *md = '*';
  for (i=0; i < SHA_DIGEST_LENGTH; i++)
    sprintf(&md[i*2+1], "%02X", hash2[i]);
}
#endif
...
#elif defined(PAM_MYSQL_PASSWORD)
					pam_mysql_password(passwd, strlen(passwd), buf);
#else
					syslog(LOG_AUTHPRIV | LOG_ERR, PAM_MYSQL_LOG_PREFIX "No support for MySQL's PASSWORD in this build");
					break;
#endif
...
#elif defined(PAM_MYSQL_PASSWORD)
				pam_mysql_password(new_passwd, strlen(new_passwd), encrypted_passwd);
#else
				syslog(LOG_AUTHPRIV | LOG_ERR, PAM_MYSQL_LOG_PREFIX "No support for MySQL's PASSWORD in this build");
#endif
				break;
